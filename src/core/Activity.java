...
clases Activity y Project, en nustro proyecto.
También actua como elemento "Element", en el patrón de diseño VISITOR, siendo la clase de la que
heredan sus "ConcretsElements".
Por otro lado, se almacenan los valores necesarios cada vez que se añade una actividad a la lista y
controla su orden de actuación dependiendo del tipo de actividad que sea; un Proyecto padre, una
Tarea hije, etc.
 */
    ...
    if (father != null) {
      if (this.getFather().getInitialDate() == null) {
        ...
        assert (getFather().getInitialDate() != getFather().getFinalDate())
            : "initialDate and finalDate are not equal at setInitialDate from Activity";
      }
    }
    assert (invariant()) : "Invariant violated";
  }
